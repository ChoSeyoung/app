...
- 광고 슬롯 바깥 카드 쉘은 서비스 디자인 시스템을 따르되, 실제 광고 영역은 추후 AdMob 네이티브 광고로 교체 가능한 구조를 유지한다.
...
- `이유식 시작하기` 같은 학습형 화면은 긴 세로 스크롤보다 `파트 선택 -> 챕터 선택 -> 현재 챕터 상세` 구조를 우선한다.
- 중요한 전환점은 `파트`로 나누고, 각 파트 안에서 챕터를 가로 접근형 카드로 빠르게 이동할 수 있어야 한다.
- 사용자는 현재 챕터를 읽음 처리하거나 책갈피로 저장할 수 있어야 하며, 그 상태는 화면 상단 요약 카드와 챕터 카드 모두에서 보여야 한다.
- 책갈피 영역은 상단 가까이에 두어 다시 볼 챕터로 빠르게 돌아갈 수 있게 한다.
- 광고 슬롯은 챕터 상세 아래처럼 흐름을 끊지 않는 지점에만 배치한다.
